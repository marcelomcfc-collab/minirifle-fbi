    ...
    <div className="flex flex-col gap-3 px-4 py-4">
      ...
      <div className="flex flex-col gap-2">
        {sessions?.map((session) => {
          ...
          const isSelected = selected.has(session.localId);
          const href = session.remoteId
            ? `/historial/${session.remoteId}`
            : `/historial/local:${session.localId}`;
          const content = (
              ...
              <div className="flex-1">
                <div className="flex items-center gap-1.5">
                  ...
                  {session.status === "pending" && (
                    <span className="rounded-full border border-accent/40 bg-accent-soft px-1.5 py-0.5 text-[9px] font-medium text-accent">
                      Sin sincronizar
                    </span>
                  )}
                </div>
          ...
          return selectionMode ? (
            <button key={session.localId} onClick={() => toggleSelected(session.localId)} className="text-left">
              {content}
            </button>
          ) : (
            <Link key={session.localId} href={href}>
              {content}
            </Link>
          );
        })}
